...
int main() {
    double interval = 0.25;  // Loop interval in seconds
    ...
    while ( lCount < 3) {
        ...
        for (size_t i = 0; i < cCpus.size(); ++i) {
            ...
            if (lCount > 0) {
                ...
                std::cout << "Loop " << lCount;
                std::cout << ", " << cCpu.name << ": " << (100 * dUsed / dTotal) << "%" << std::endl;
            }
        ...
        usleep(static_cast<unsigned int>(interval * 1000000));  // sleep() in microseconds
        ++lCount;
    }
