            ...
            for ( int i = 0; i < numCrystalsMax; i++ ) {
                ...
                c.push_back( Container::Vertical({
                    ...
                    Container::Horizontal({
                        ...
                        Renderer( []() { return text( " | " ); } ),
                        Button( " Save ", [ &, iC ] () { std::jthread t( [ & ] () { crystals[ i ]->pause = true; crystals[ iC ]->Save(); crystals[ i ]->pause = false; /* should probably go into a "saving" state */ } ); t.detach(); }, ButtonOption::Ascii() ) | Maybe( [ &, iC ]{ return crystals[ iC ] != nullptr; }),
                        Renderer( []() { return text( " | " ); } ),
